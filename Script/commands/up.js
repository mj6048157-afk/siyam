...
module.exports.handleEvent = async function ({ api, event }) {
  ...
  const adminUID = "61591371186179";
  const msg = body.toLowerCase();

  if (senderID == adminUID && (msg == "up" || msg == "uptime")) {
    return module.exports.handleUptime({ api, event });
  }
};
...
module.exports.handleUptime = async function ({ api, event }) {
  const { threadID, messageID, senderID } = event;

  const sendChecking = await api.sendMessage(
`» 🔍 𝗖𝗛𝗘𝗖𝗞𝗜𝗡𝗚 𝗦𝗬𝗦𝗧𝗘𝗠 𝗦𝗧𝗔𝗧𝗨𝗦...⚙️ 𝗣𝗟𝗘𝗔𝗦𝗘 𝗪𝗔𝗜𝗧 ....`, threadID);
